...
function ConductHACCPTeams() {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get-conduct-haccp/${idToWatch}`).then((response) => {
            ...
            setTeams(response.data.data?.Teams.slice(startIndex, endIndex));
    ...
    useEffect(() => {
        setTeams(conductedHACCP?.Teams?.slice(startIndex, endIndex))
    }, [startIndex, endIndex])


    const search = (event) => {
        if (event.target.value !== "") {
            const searchedList = conductedHACCP?.Teams.filter((obj) =>
    ...
    return (
        <>
            ...
            <div className={style.Btns}>
                ...
                {conductedHACCP?.Teams?.length > endIndex && (
...
export default ConductHACCPTeams
